...
namespace godot
{
	...
	int Emoto::copy_buffer()
	{
		...
		uint8_t* _image_data = m_image_data.write().ptr();
		...
		if (ResourceDesc.pData)
		{
			...
			for (int i = 0; i < TEXTURE_SIZE_Y; ++i)
			{
				std::memcpy(
					(byte*)_image_data + TEXTURE_SIZE_X * BytesPerPixel * i,
					(byte*)ResourceDesc.pData + ResourceDesc.RowPitch * i,
					TEXTURE_SIZE_X * BytesPerPixel);
			}
		}
